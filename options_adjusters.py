...
def get_all_products():
    products = []
    page = 1
    limit = 50  # BigCommerce default page size
        ...
        for modifier in modifiers:
            if modifier.get("name") == "Ships Every":
                ...
                for option in option_values:
                    adjusters = option.get("adjusters", "None")
                    results.append({
                        "Product ID": product_id,
                        "Product Name": product.get("name"),
                        "Modifier Name": "Ships Every",
                        "Adjusters": adjusters if adjusters else "None"
                    })
    return results

def export_to_csv(data, filename="products_with_adjusters.csv"):
    with open(filename, mode='w', newline='') as file:
        writer = csv.DictWriter(file, fieldnames=["Product ID", "Product Name", "Modifier Name", "Adjusters"])
        writer.writeheader()
...
def main():
    ...
    print("Filtering products with 'Ships Every' modifier...")
    ...
    if filtered_products:
        ...
        print("Export completed: products_with_adjusters.csv")
    else:
